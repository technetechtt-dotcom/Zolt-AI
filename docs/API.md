...
Machine-readable contract: [`openapi.yaml`](./openapi.yaml). All `/v1` endpoints require a database-backed API credential or tenant-bound bearer session except identity bootstrap/recovery endpoints. `ZOLT_API_KEY` is rejected in production.

API and user credentials are tenant-scoped. Product/installation scope and user installation grants are checked before lookup; object-ID routes recheck ownership to prevent IDOR. High-risk credentials require administrator approval and every credential expires within policy.

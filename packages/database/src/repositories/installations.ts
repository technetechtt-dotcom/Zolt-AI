...
export async function resolveInstallationIdentity(ref: IdentityRef): Promise<IdentityIds> {
  ...
  if (installation.tenantId !== tenant.id || installation.productId !== product.id) {
    throw new Error("TENANT_ISOLATION_VIOLATION");
  }
  return { tenantId: tenant.id, productId: product.id, installationId: installation.id };
}
...
export async function resolveProductIdentity(input: {
  ...
  const tenant = await db().tenant.findUnique({ where: { id: input.tenantKey } });
  if (!tenant) {
    throw new Error("INSTALLATION_IDENTITY_NOT_FOUND");
  }
  const product = await db().product.findFirst({
...
export async function ensureInstallationIdentity(ref: IdentityRef): Promise<IdentityIds> {
  try {
  ...
  const product = await db().product.upsert({
    where: { tenantId_externalKey: { tenantId: tenant.id, externalKey: ref.productKey } },
...
export async function assertTenantOwns(tenantId: string, ids: IdentityIds): Promise<void> {

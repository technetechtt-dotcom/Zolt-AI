...
import { assertAdvisoryOnlyRuntime, assertNoPlantCommand, HARDWARE_EXECUTION_FORBIDDEN } from "@zolt/safety";
...
export function deduplicationKey(parts: Record<string, string | undefined>): string {
...
export class AnalysisOrchestrator {
  ...
  async analyse(context: ZoltContext): Promise<ZoltRecommendation[]> {
    ...
    for (const skill of this.skills.filter((candidate) => candidate.supports(context))) {
      try {
        ...
        const confidence = calibrateConfidence(result.recommendation.confidenceBreakdown);

...
import {
  ...
  type TelemetryValidationProfile,
} from "@zolt/core";
import { instrumentFastify, metrics } from "@zolt/observability";
...
export interface GatewayDependencies {
  ...
  verifyTenantAccess: (identity: {
    ...
    boolean | { tenantId: string; productId: string; installationId: string }
  >;
  getValidationProfile?: (identity: {
    tenantId: string;
    installationId: string;
    assetId?: string;
  }) => Promise<TelemetryValidationProfile | undefined>;
...
export function buildGatewayApp(
  ...
  instrumentFastify(app, "zolt-connector-gateway");
  ...
  app.get("/health/live", async () => ({
    status: "ok",
    service: "zolt-connector-gateway",
  }));
  app.get("/health/ready", async (_req, reply) => {
    ...
    if (!ready) {
      return reply
        .code(503)
        .send({ status: "degraded", service: "zolt-connector-gateway" });
    }
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const tenantId = String(
        h["x-zolt-tenant-id"] ?? req.zoltAuth?.tenantId ?? "",
      );
      const productId = String(
        h["x-zolt-product-id"] ?? req.zoltAuth?.productId ?? "",
      );
      const installationId = String(
        h["x-zolt-installation-id"] ?? req.zoltAuth?.installationId ?? "",
      );
      if (!tenantId || !productId || !installationId) {
        return reply.code(401).send({ code: "MISSING_INTEGRATION_IDENTITY" });
      }
      const access = await dependencies.verifyTenantAccess({
        tenantId,
        productId,
        installationId,
      });
      ...
      const boundIdentity =
        typeof access === "object"
          ? access
          : { tenantId, productId, installationId };
      try {
        if (req.zoltAuth) assertIdentityBinding(req.zoltAuth, boundIdentity);
      } catch {
      ...
      for (const message of messages) {
        const profile = await dependencies.getValidationProfile?.({
          tenantId: boundIdentity.tenantId,
          installationId: boundIdentity.installationId,
          assetId: message.assetId,
        });
        const checked = validateTelemetryEnvelope(message, undefined, profile);
        if (checked.errors.length > 0 || !checked.envelope) {
          return reply
            .code(400)
            .send({ code: "INVALID_TELEMETRY", errors: checked.errors });
        }
        if (
          checked.envelope.tenantId !== boundIdentity.tenantId ||
          checked.envelope.productId !== boundIdentity.productId ||
          checked.envelope.installationId !== boundIdentity.installationId
        ) {
          return reply.code(403).send({ code: "CONNECTOR_SCOPE_MISMATCH" });
        }
...
export async function startGatewayServer(): Promise<void> {
  assertProductionConfiguration("gateway");
  ...
  const dependencies: GatewayDependencies = {
    ...
    verifyTenantAccess: async (identity) => {
      try {
        return await database.resolveInstallationIdentity({
    ...
    getValidationProfile: database.getTelemetryValidationProfile,
  ...
  await app.listen({
    port: Number(process.env.CONNECTOR_GATEWAY_PORT ?? 4001),
    host: "0.0.0.0",
  });
}

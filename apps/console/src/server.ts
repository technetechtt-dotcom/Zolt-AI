...
export function buildConsoleApp() {
  ...
  const html = consoleHtml(process.env.ZOLT_API_BASE_URL ?? "http://localhost:4000");
  app.get("/health/live", async () => ({ status: "ok", service: "zolt-console" }));
...
export async function startConsoleServer(): Promise<void> {
  ...
  await app.listen({ port: Number(process.env.CONSOLE_PORT ?? 4002), host: "0.0.0.0" });
}
